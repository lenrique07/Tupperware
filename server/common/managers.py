...
class SuperManager:
    ...
    def  bitacora(self, object):
        from ..operaciones.managers import BitacoraManager
        from ..operaciones.managers import Bitacora
        fecha = BitacoraManager(self.db).fecha_actual()
        b = Bitacora(fkusuario=object.user_id, ip=object.ip, accion=object.nombre, fecha=fecha)
        SuperManager(self,self.db).insert(b)
        return b
    def imagen(self,fileinfo):

        fname = fileinfo.filename
        extn = os.path.splitext(fname)[1]
        cname = str(uuid.uuid4()) + extn
        f = open("./server/common/resources/images/" + cname, 'wb')
        f.write(fileinfo.body)
        f.close()
        foo = Image.open("./server/common/resources/images/" + cname)
        foo = foo.resize((600, 600), Image.ANTIALIAS)
        print(foo)
        imgByteArr = io.BytesIO()
        foo.save(imgByteArr, format='jpeg')
        imgByteArr = imgByteArr.getvalue()
        cliente = dropbox.Dropbox('v0XIGaIghHAAAAAAAAAD7T_jN4-pBkDTAT0jxzseAo2sHSt3RWsHD7a-SKfWi5ka')
        # CORRECCION DROPBOX
        cname = 'a' + cname
        respuesta = cliente.files_upload(imgByteArr, '/images/' + cname)
